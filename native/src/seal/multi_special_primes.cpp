...
void modup_to_single_rns(
    const uint64_t *in_poly, uint64_t *dst_poly, const size_t degree, const std::vector<size_t> &in_poly_rns_indices,
    const size_t dst_rns_index, const std::vector<Modulus> &key_rns, MemoryPool &pool)
{
    ...
    if (in_poly_rns_indices.size() == 1)
    {
        ...
        else
        {
            ...
            std::transform(in_poly, in_poly + degree, dst_poly, [rns](uint64_t v) -> uint64_t {
...
void modup_rns(
    const uint64_t *src_poly, uint64_t *dst_poly, const size_t degree, const size_t n_ct_rns, const size_t n_sp_rns,
    const size_t src_bundle_index, std::vector<Modulus> const &key_modulus, MemoryPool &pool)
{
...
std::vector<uint64_t> puncture_products_of_special_rns(
    std::vector<Modulus> const &key_rns, size_t n_special_rns, size_t normal_rns_index)
{
...
std::vector<uint64_t> inv_puncture_products_of_special_rns(std::vector<Modulus> const &key_rns, size_t n_special_rns)
{
...
void rescale_special_rns_inplace(
    uint64_t *poly, bool is_ckks, const size_t degree, const size_t n_ct_rns, const size_t n_sp_rns,
    std::vector<Modulus> const &key_rns, const NTTTables *small_ntt_tables, MemoryPool &pool)
{
    ...
    for (size_t i = 0; i < n_ct_rns; ++i)
    {
        ...
        SEAL_ITERATE(
            iter(poly + i * degree, temp_poly.get()), degree, [qi_lazy](auto K) { get<0>(K) += qi_lazy - get<1>(K); });
    ...
    MemoryPoolHandle pool)
{
    ...
    for (size_t src_bundle_idx = 0; src_bundle_idx < n_bundles; ++src_bundle_idx)
    {
        ...
        SEAL_ITERATE(
            iter(dst_iter + rns0, target_iter + rns0, small_ntt_tables + rns0), rns1 - rns0, [degree, is_ckks](auto I) {
                set_uint(get<1>(I), degree, get<0>(I));
                if (is_ckks)
                    inverse_ntt_negacyclic_harvey(get<0>(I), get<2>(I));
            });

        // Step 2: modulus up, including all normal primes and special primes.
        modup_rns(
            poly_ext_rns.get() + rns0 * degree, poly_ext_rns.get(), degree, n_ct_rns, n_special_rns, src_bundle_idx,
            key_modulus, pool);
    ...
    for (size_t b : { 0, 1 })
    {
        ...
        rescale_special_rns_inplace(
            cmult_evk_ptr, is_ckks, degree, n_ct_rns, n_special_rns, key_modulus, small_ntt_tables, pool);
