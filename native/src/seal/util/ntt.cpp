...
namespace seal
{
    namespace util
    {
        ...
        void NTTTables::initialize(int coeff_count_power, const Modulus &modulus)
        {
            ...
            inv_root_powers_[coeff_count_ - 1] =
                multiply_uint_mod(inv_root_powers_[coeff_count_ - 1], inv_degree_modulo_, modulus_);
        ...
        struct SlothfulNTT
        {
            ...
            explicit SlothfulNTT(uint64_t p, uint64_t Lp, uint64_t rdp) : p(p), Lp(Lp), rdp(rdp)
            {
                // if (p >= (1ULL << 59))
                // {
                //     throw std::logic_error("SlothfulNTT: |p| out-of-bound. 59bit most");
                // }
            }
            ...
            // x0' <- x0 + w * x1 mod p
            // x1' <- x0 - w * x1 mod p
            inline void Forward(uint64_t *x0, uint64_t *x1, uint64_t w, uint64_t wshoup) const
            {
                uint64_t u, v;
                u = *x0;
            ...
            inline void ForwardLast(uint64_t *x0, uint64_t *x1, uint64_t w, uint64_t wshoup) const
            {
                uint64_t u, v;
            ...
            // x0' <- x0 + x1 mod p
            // x1' <- x0 - w * x1 mod p
            inline void Backward(uint64_t *x0, uint64_t *x1, uint64_t w, uint64_t wshoup) const
            {
                uint64_t u = *x0;
                uint64_t v = *x1;
                uint64_t t = u + v;
            ...
            inline void BackwardLast(
                uint64_t *x0, uint64_t *x1, uint64_t inv_n, uint64_t inv_n_s, uint64_t inv_n_w,
                uint64_t inv_n_w_s) const
            {
                uint64_t u = *x0;
                uint64_t v = *x1;
                uint64_t t = u + v;
        ...
        */
        void ntt_negacyclic_harvey_lazy(CoeffIter operand, const NTTTables &tables)
        {
            ...
            { // unrolling
                sntt.Forward(x0++, x1++, *w, *wshoup);
                sntt.Forward(x0, x1, *w, *wshoup); // combine the incr to following steps
                x0 += 3;
                x1 += 3;
            }
            ...
            for (size_t r = 0; r < m; ++r, ++w, ++wshoup)
            {
                sntt.ForwardLast(x0, x1, *w, *wshoup);
                x0 += 2;
                x1 += 2;
            }
        ...
        void inverse_ntt_negacyclic_harvey_lazy(CoeffIter operand, const NTTTables &tables)
        {
            ...
            {
                sntt.BackwardLast(x0++, x1++, inv_n, inv_n_s, *w, *wshoup);
            }
        }
    } // namespace util
} // namespace seal
